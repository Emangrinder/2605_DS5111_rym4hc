"""Offline mock test suite for the Snowflake loader pipeline."""
import sys
import io
import json
...
def test_load_snowflake_pipeline_ingestion_loop(monkeypatch):
    ...
    assert mock_cursor.execute.call_count >= 3, (
        "The database cursor should trigger table generation plus one "
        "call per row entry."
    )

    # Extract the distinct raw query text argument configurations executed
    # by the mock engine
    executed_queries = [
    ...
    for query in insert_queries:
        assert "PARSE_JSON(%s)" in query, (
            "The engine must pass payloads safely wrapped inside a "
            "database PARSE_JSON call."
        )
    ...
    # Ensure parameter payloads are passed safely within matching
